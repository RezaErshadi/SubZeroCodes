...
def InitiateAllNodes(AllNodes,logger):
    ...
    for i in range(len(AllNodes)):
        ...
        subprocess.Popen(f"ros2 run sar_rtk {n}",shell=True,stdin=None,stdout=None,stderr=None,close_fds=True)
        time.sleep(0.5)
...
LogFileName = f"{nf+1}_NBsarrtk_{now.strftime('%m-%d-%Y_%H-%M-%S')}.txt"
...
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
AllNodes = ['logall','trimble','apres','brain']
now = datetime.datetime.now()
print(now)
